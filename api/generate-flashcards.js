...
const HEADLINE_RULES = {
    ...
    B: `  The FIRST card for each sub-topic MUST be its HEADLINE card, and for temporal cards it anchors the sub-topic on the timeline: describe the sub-topic's single defining event, turning point, or development and ask roughly WHEN it happened. Its ANSWER must be a TIME -- a rough date, century, era, or "X ago" magnitude (e.g. "14th century", "about 3 billion years ago") -- NOT the event itself, and NOT a mere restatement of the sub-topic's own dates or label from its title. Keep the answer short. Never make the headline trivia.`,
...
const breadthGuidanceFor = (breadth) => ({
    ...
    B: "These are TEMPORAL cards -- their job is to build the learner's mental timeline, so for MOST cards the ANSWER should be a TIME rather than an event or concept: a rough date, century, era, or 'X ago' magnitude; a duration; or a chronological ordering. Describe an event or development from the narrative in the question and ask roughly WHEN it happened, HOW LONG it lasted, or WHICH of two things came first. Aim for rough placement in time, not exact-date drilling. Cards whose answer is an event, person, or concept (rather than a time) should be a small minority here -- those belong to the analytical breadth.",
