...
export async function generateCards(anchor, children, research, frame = null, model = CARDS_MODEL) {
    ...
    // A 'when' section is normally whole-world, but when the anchor being studied sits under (or is)
    // a geographic (C) anchor, frame.geoScope bounds it to that region instead (see buildSectionFrame
    // in lib/learnContent.js and learnCoverageInstruction in lib/promptLoader.js) — e.g. studying
    // Oceania's time windows must stay about Oceania, not broaden to the whole world.
    const subjectFor = (i) => {
        const s = frame?.subjects?.[i];
        if (s?.axis === 'when' && frame?.geoScope) return `A TIME WINDOW. Cover ${frame.geoScope} during ${s.coord}. The section's name is only a signpost for the period — do not narrow the cards to it, and do not broaden beyond ${frame.geoScope}.`;
