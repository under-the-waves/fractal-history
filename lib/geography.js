...
const subregionOf = new Map(); // cca2 -> subregion code
const regionOf = new Map(); // cca2 -> region code
const areaOf = new Map(); // cca2 -> land area in km² (0 when the dataset has none)
...
function buildTaxonomy() {
  ...
  for (const country of countries) {
    ...
    subregionOf.set(cca2, subregion);
    regionOf.set(cca2, region);
    areaOf.set(cca2, Number(country.area) || 0);
...
export function getSubregion(cca2) {
  return subregionOf.get(cca2) ?? null;
}

export function getRegion(cca2) {
  return regionOf.get(cca2) ?? null;
}

export function getArea(cca2) {
  return areaOf.get(cca2) ?? 0;
}
...
// Resolve a C-breadth anchor's region_codes to its display member codes. Legacy anchors stored a
// region/subregion name (pre country-grouping) and need expanding down to countries; current
// anchors already store cca2 / ISO 3166-2 codes directly and pass through unchanged. Shared by
// api/get-tree.js (member list on hover cards) and lib/pageGeo.js (learning-page atlas maps).
export function resolveMemberCodes(codes) {
  if (!Array.isArray(codes) || codes.length === 0) return [];
  const isLegacy = codes.some((c) => {
    const lvl = getLevel(c);
    return lvl === 'region' || lvl === 'subregion';
  });
  return isLegacy ? [...expandToCountries(codes)] : codes;
}
