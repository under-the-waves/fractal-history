...
/**
 * Like getAncestorDistances, but also returns, for each ancestor, the breadth of the EDGE leading down
 * toward this anchor — i.e. the breadth of the ancestor's child on the shortest path. Used by the
 * per-pathway score breakdown so a descendant's whole score is attributed to the A/B/C pathway it sits
 * under, not the breadth of the individual narrative studied. The anchor itself is at dist 0 with
 * edgeBreadth null (its own score is split by card breadth, not by an edge).
 *
 * Each step up the tree carries the child's breadth forward: stepping from position X to its parent P,
 * the edge under P toward the anchor is X's breadth.
 *
 * @returns {Promise<Array<{id: string, dist: number, edgeBreadth: string|null}>>}
 */
export async function getAncestorEdges(anchorId) {
    const rows = await query(`
        WITH RECURSIVE up AS (
            SELECT tp.position_id, tp.anchor_id, tp.parent_position_id,
                   tp.breadth AS my_breadth, NULL::text AS edge_breadth, 0 AS dist
            FROM tree_positions tp
            WHERE tp.anchor_id = $1
            ...
            SELECT p.position_id, p.anchor_id, p.parent_position_id,
                   p.breadth AS my_breadth, up.my_breadth AS edge_breadth, up.dist + 1
            FROM up
            JOIN tree_positions p ON p.position_id = up.parent_position_id
            WHERE up.dist < 50
        )
        SELECT DISTINCT ON (anchor_id) anchor_id AS id, dist, edge_breadth
        FROM up
        ORDER BY anchor_id, dist ASC
    `, [anchorId]);
    return rows.map(r => ({ id: r.id, dist: Number(r.dist), edgeBreadth: r.edge_breadth }));
}
...
const CONCEPT_STOPWORDS = new Set([
    'the', 'a', 'an', 'of', 'on', 'in', 'to', 'and', 'for', 'at', 'by', 'with', 'from', 'as'
]);

/**
 * Normalise an A-concept title for reuse matching: lowercase, strip punctuation, drop stopwords, sort
 * the remaining tokens. So "The Emergence of Life on Earth" and "Emergence of Life on Earth" and
 * "Life, Earth: Emergence" all reduce to the same key. Deterministic and free — it catches formatting,
 * word-order and stopword variants, but NOT genuine paraphrase ("Emergence of Life" vs "Life Emerges &
 * Evolves"), which is deliberately left to a future semantic/LLM pass to avoid false merges.
 */
export function normaliseConceptTitle(title) {
    return (title || '')
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, ' ')
        .split(/\s+/)
        .filter(w => w && !CONCEPT_STOPWORDS.has(w))
        .sort()
        .join(' ');
}

/**
 * Every global A (analytical/concept) anchor: { id, title }. A concepts are the reusable, path-
 * independent unit — reachable via any pathway and the same thing wherever they appear. B and C anchors
 * are parent-scoped coordinate slices (a time window / a place), so they are NOT candidates for reuse.
 * "Global" = no region_codes, which analytical anchors always are.
 */
export async function getGlobalConceptAnchors() {
    const rows = await query(`
        SELECT DISTINCT a.id, a.title
        FROM anchors a
        JOIN tree_positions tp ON tp.anchor_id = a.id
        WHERE tp.breadth = 'A'
          AND (a.region_codes IS NULL OR a.region_codes = '[]'::jsonb)
    `);
