...
function factLines(facts) {
    return (facts || []).map(f => {
        ...
        (f.before || []).forEach(b => lines.push(`    · before: ${b}`));
        (f.what || []).forEach(b => lines.push(`    · ${b}`));
        (f.why || []).forEach(b => lines.push(`    · why: ${b}`));
        (f.whyItMattered || []).forEach(b => lines.push(`    · why it mattered: ${b}`));
...
export function buildNarrativeGrounding(content) {
    ...
    if (content.postlude) secs.push(`## Why it mattered — ${content.postlude.title}\n${factLines(content.postlude.facts)}`);
