...
// `geoScope` (from frame.geoScope) bounds a 'when' section to a region — e.g. Oceania — when the
// anchor being studied sits under (or is) a geographic (C) anchor; null means whole-world, unchanged.
function sectionAim(sec, geoScope) {
    if (sec.isPrelude) return sec.key;
    if (sec.axis === 'when' && geoScope) return `${sec.key} — RESEARCH ${geoScope} during ${sec.coord} (the descriptive name is only a signpost for the period; do not narrow to it, and do not broaden beyond ${geoScope})`;
...
function extractionAim(sec, geoScope) {
    if (sec.axis === 'when' && geoScope) return `everything significant that happened in ${geoScope} during ${sec.coord}`;
...
export async function researchAnchor(anchor, children, frame = null) {
    ...
    const geoScope = frame?.geoScope || null;
    ...
    const queriesText = await llm(
        ...
        `${querySpec.map((s, i) => `${i + 1}. ${sectionAim(s, geoScope)}`).join('\n')}\n\n` +
    ...
    await Promise.all(querySpec.map(async (sec) => {
        ...
        const evText = await llm(
            `Topic: ${anchorTopic}. This section: ${extractionAim(sec, geoScope)}.\n\n` +
            `From the search snippets and source pages below, extract the key factual points CLEARLY supported by them, ` +
            `covering ${extractionAim(sec, geoScope)}. Include SPECIFIC dates, numbers, names and places wherever the ` +
