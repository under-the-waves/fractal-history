 ...
 */
export async function researchAnchor(anchor, children) {
    ...
    let querySpec;
    try { querySpec = parseJson(queriesText).sections || []; }
    catch { querySpec = sections.map(s => ({ title: s, queries: [s] })); }
    ...
    const preludeEvidence = preludeKey ? (evidenceBySection[preludeKey] || []) : [];
