 ...
 * value 0..B. FLASHCARD MASTERY ONLY — deliberately narrower than computeOwnRaw. This is the
 * achievement engine's definition of "mastered" (lib/achievements.js MASTERY_THRESHOLD), which is
 * specifically about card retention, not narrative writes; do not fold the write term in here, or
 * banked mastery and achievement stats will shift. For the UI's own-mastery display (cards + writes),
 * see computeOwnBreadthWithWrite below, which builds on this function rather than re-querying.
 * One grouped query.
 *
 * `applyFloor`: when true, adds computeOwnRaw's FLOOR_RETENTION cutoff (a card's retention below 0.2
 * counts as fully lapsed, i.e. 0). Defaults to false so the achievement engine's existing callers see
 * unchanged behaviour (this floor predates the achievement mastery banking and must not shift it);
 * computeOwnBreadthWithWrite passes true so the score display matches computeOwnRaw exactly.
 */
export async function computeBreadthBreakdown(userId, { applyFloor = false } = {}) {
    const rows = applyFloor
        ? await sql`
...
/**
 * Per-breadth OWN score for every anchor the user has engaged, INCLUDING the narrative-write term on
 * top of the flashcard term from computeBreadthBreakdown — mirroring computeOwnRaw's per-breadth maths
 * exactly (0..B for cards + 0..WRITE_FULL for writing = 0..B+WRITE_FULL per breadth). This is what the
 * tree UI reads for the own-mastery bar under each A/B/C tile.
 *
 * Also returns `written`: { anchorId: { A?: true, B?: true, C?: true } } marking which (anchor,
 * breadth) pairs have a stored write mark AT ALL, regardless of how far it has decayed — the "written
 * here" tick is about whether the user has ever written this level's narrative, not its current value.
 *
 * Deliberately separate from computeBreadthBreakdown (used by the achievement engine's flashcard-only
 * mastery threshold) so extending the UI's display never shifts what counts as "mastered" there.
 */
export async function computeOwnBreadthWithWrite(userId) {
    const [cardOwn, writeRows] = await Promise.all([
        computeBreadthBreakdown(userId, { applyFloor: true }),
        sql`
            SELECT anchor_id, breadth, COALESCE(SUM(
                LEAST(${WRITE_FULL}, (score / 100.0) * ${WRITE_FULL}) * (
                    CASE
    ...
    const own = {};
    for (const [anchorId, byB] of Object.entries(cardOwn)) own[anchorId] = { ...byB };
    const written = {};
    for (const r of writeRows) {
        (own[r.anchor_id] ||= {});
        own[r.anchor_id][r.breadth] = (own[r.anchor_id][r.breadth] || 0) + Number(r.write_own);
        (written[r.anchor_id] ||= {})[r.breadth] = true;
    }
    return { own, written };
}
 ...
 *
 * `precomputedOwn` lets a caller that already ran computeOwnBreadthWithWrite (e.g. api/scores.js, for
 * the own-mastery display) pass its `own` map straight in, so the card + write queries run once, not
 * twice, per request. Falls back to computing it when called standalone.
 */
export async function computeBreadthSubtreeBreakdown(userId, precomputedOwn) {
    // Per-breadth OWN score for every engaged anchor: flashcard retention (capped at B per breadth)
    // plus write-your-own marks (added on top) — the exact same own map computeOwnBreadthWithWrite
    // returns, reused here rather than re-queried.
    const own = precomputedOwn || (await computeOwnBreadthWithWrite(userId)).own;
