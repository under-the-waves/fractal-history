...
export async function computeOwnRaw(userId, anchorId) {
    ...
    const marks = await sql`
        SELECT score FROM learn_marks
        WHERE user_id = ${userId} AND anchor_id = ${anchorId}
    `;
    for (const m of marks) own += Math.min(WRITE_FULL, (Number(m.score) / 100) * WRITE_FULL);
...
/**
 * Persist a write-your-own mark (keeping the user's BEST score for this anchor/breadth) and fold it
 * into the score cache. Upserts learn_marks, then runs applyReviewDelta — which recomputes own_raw via
 * computeOwnRaw (now including the write term) and propagates the change up the ancestor chain. Returns
 * the write XP this breadth now contributes and the node's resulting subtree score.
 */
export async function recordWriteMark(userId, anchorId, breadth, score, covered = 0, total = 0) {
    await sql`
        INSERT INTO learn_marks (user_id, anchor_id, breadth, score, covered, total, attempts, updated_at)
        VALUES (${userId}, ${anchorId}, ${breadth}, ${score}, ${covered}, ${total}, 1, NOW())
        ON CONFLICT (user_id, anchor_id, breadth) DO UPDATE SET
            score = GREATEST(learn_marks.score, EXCLUDED.score),
            covered = CASE WHEN EXCLUDED.score >= learn_marks.score THEN EXCLUDED.covered ELSE learn_marks.covered END,
    ...
    const writeXp = Math.min(WRITE_FULL, (Number(score) / 100) * WRITE_FULL);
    const node = await getUserNodeScore(userId, anchorId);
    return { writeXp: Math.round(writeXp), nodeScore: node.display };
}
