 ...
 * reached through — the same roll-up recomputeUserScores does, but carried in three buckets instead
 * of one, so A + B + C == the node's subtree_raw.
 *
 * It works because own(node) already decomposes cleanly by breadth: the flashcard term is a per-breadth
 * sum capped per breadth, and each write mark carries its own breadth. So own_X(d) is well defined for
 * every engaged descendant d, and subtree_X(node) = Σ_d R^dist(node,d) * own_X(d). Summing the three
 * gives exactly Σ_d R^dist * own(d) = subtree_raw. Each descendant is counted once at its SHORTEST
 * distance (getAncestorDistances), so reuse never double-counts — identical to the single-bucket roll-up.
 */
export async function computeBreadthSubtreeBreakdown(userId) {
    ...
    // Roll each engaged node's per-breadth own score up to itself and every ancestor, discounted R^dist.
    const subtree = {};
